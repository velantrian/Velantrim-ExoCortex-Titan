...
def test_total_byte_budget_counts_rejected_reads_before_later_files(
    tmp_path: Path,
    monkeypatch,
) -> None:
...
def test_growth_after_stat_is_read_only_to_budget_plus_detection_byte(
    tmp_path: Path,
    monkeypatch,
) -> None:
    conn, root = _registered(tmp_path)
    try:
        ...
        assert outcome.promoted is False
        assert outcome.final_disposition == "INCOMPLETE_REJECTED"
        assert len(victim.read_bytes()) > 17
