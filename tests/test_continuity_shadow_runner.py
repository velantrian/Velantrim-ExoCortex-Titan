"""End-to-end tests for the disabled-by-default complete shadow runner."""
...
from core.compute_controller import ContextFreshness, ContinuityComputeSignals
    ...
    return CompleteShadowRunInput(
        ...
        advisory_intents=(
            AdvisoryIntent(
                ...
                advisory_target or assertion.assertion_id,
            ),
        ),
...
def _enabled_config() -> ShadowRunnerConfig:
    return ShadowRunnerConfig(
        enabled=True,
        scenario_id="milestone-1-complete-shadow",
    )
...
def test_complete_real_pipeline_replays_and_emits_shadow_confirmation() -> None:
...
def test_external_input_order_does_not_change_result_identity() -> None:
    inputs = _input()
    reordered = replace(inputs, episodes=tuple(reversed(inputs.episodes)))
