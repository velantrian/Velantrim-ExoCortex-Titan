"""Latency-architecture invariants for PR-ARM-03 selective-memory shadow work."""
...
def test_flag_off_never_invokes_candidate_extraction(monkeypatch):
    """The default path must return before any candidate scan is attempted."""
...
def test_default_policy_is_strictly_bounded():
...
def test_extractor_has_no_async_model_or_network_execution_surface():
    """PR-ARM-03 stays a cheap local rule baseline, not a hidden slow path."""
...
def test_truncation_bounds_output_even_for_large_input():
    text = " ".join(f"I prefer deterministic option {index}." for index in range(500))
    policy = smc.CandidateExtractionPolicy(
        max_source_spans=20,
    ...
    assert sum(len(candidate.normalized_text) for candidate in result.candidates) <= 180
    assert result.trace.canon_write_count == 0
    assert result.trace.memory_write_count == 0
