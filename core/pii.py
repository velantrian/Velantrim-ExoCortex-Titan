# core/pii.py
# VELANTRIM Titan — PII Detection & Redaction (GDPR Art. 5 data minimisation)
#
# Detects and redacts common personal data in free text before it is stored, so
# memory keeps only what it needs ("data minimisation", Art. 5(1)(c)) and avoids
# retaining raw identifiers. Detection is heuristic and dependency-free (stdlib
# `re` only) — consistent with Titan's stdlib-core principle.
#
...
# redact() helpers are always available. Findings are content-free (type + span),
# never the matched value itself.
#
# Ported from the Crystal open core (core/pii.py) — kept identical so the open
# library and the Titan product share one verified implementation.
