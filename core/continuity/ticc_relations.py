...
class TICCRelationRequest:
    ...
    source_candidate: TICCCaptureCandidate
    ...
    def __post_init__(self) -> None:
        if (
            not isinstance(self.relation_type, AssertionRelationType)
            or self.relation_type not in _ALLOWED_TYPES
        ):
            raise TICCRelationError("only CORRECTS/RETRACTS are permitted in v0")
        if not isinstance(self.source_candidate, TICCCaptureCandidate):
            raise TICCRelationError("source_candidate must be TICCCaptureCandidate")
        expected_modality = {
            AssertionRelationType.CORRECTS: TICCSemanticModality.CORRECTION,
            AssertionRelationType.RETRACTS: TICCSemanticModality.RETRACTION,
        }[self.relation_type]
        if self.source_candidate.semantic_modality is not expected_modality:
            raise TICCRelationError("source candidate modality does not match relation type")
        if (
            self.source_candidate.disposition is not TICCDisposition.DEFERRED
            or TICCReasonCode.RELATION_NOT_IMPLEMENTED
            not in self.source_candidate.reason_codes
        ):
            raise TICCRelationError("source candidate must be a deferred revision")
        ...
        if self.source_candidate.source_ref not in self.source_assertion.source_refs:
            raise TICCRelationError("source assertion is not bound to source candidate")
        if self.source_assertion.asserted_by != self.source_candidate.actor_ref:
            raise TICCRelationError("source assertion actor does not match source candidate")
        if (
            self.source_candidate.origin_type is None
            or self.source_assertion.origin_type is not self.source_candidate.origin_type
        ):
            raise TICCRelationError("source assertion origin does not match source candidate")
        ...
        if self.source_candidate.source_ref not in refs:
            raise TICCRelationError("evidence_refs must include source candidate reference")
