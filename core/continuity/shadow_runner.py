"""Disabled-by-default orchestration for the complete Continuity shadow path.

R5B composes the independently rebuilt R1-R5A components in memory. The
runner has no startup registration, API route, persistence, Canon mutation,
answer, delivery, tool, network, or action interface. ``enabled=True`` is an
explicit local call permission to evaluate shadow artifacts; it is not runtime
activation.
"""
...
SHADOW_RUNNER_SCHEMA_VERSION = "continuity.complete_shadow_runner.v2"
SHADOW_RUNNER_POLICY_VERSION = "continuity.complete_shadow_runner.policy.v2"

_T = TypeVar("_T")


class CompleteShadowRunnerError(ValueError):
    """The complete shadow runner received invalid or ambiguous input."""
...
class ShadowRunnerConfig:
    """Explicit local evaluation config; never a runtime feature flag."""

    enabled: bool = False
    scenario_id: str = "titan-continuity-complete-shadow"
...
class ShadowGatePolicyTemplate:
    """Caller-supplied typed policy facts copied into Gate records."""
...
class AdvisoryIntent:
    """Explicit semantic target resolved exactly after projections exist."""
...
class ShadowPassArtifacts:
    ...
    compute_assessment: ContinuityComputeAssessment
...
class CompleteShadowRunReceipt:
    ...
    def create(
        ...
        *,
        ...
        request = _text(request_ref, "request_ref") if request_ref else None
        baseline_id = (
        ...
        boundary_reasons = {
            ...
            ShadowRunnerReason.NO_RUNTIME_AUTHORITY,
        }
        ...
        if status is ShadowRunnerStatus.DISABLED:
            if any(value is not None for value in artifact_refs):
                raise CompleteShadowRunnerError(
        ...
        elif any(value is None for value in artifact_refs):
            raise CompleteShadowRunnerError(
...
class CompleteShadowRunner:
    """Compose one complete deterministic in-memory shadow evaluation."""
    ...
    def _run_pass(
        ...
        *,
        ...
        compute_assessment = assess_compute_with_continuity(
            inputs.query,
            signals=inputs.compute_signals,
            candidate_count=len(candidates),
        )
        compute_decision = compute_assessment.decision
