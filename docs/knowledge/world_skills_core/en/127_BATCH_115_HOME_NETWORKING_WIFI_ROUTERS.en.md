...
|----|---------------|-----|------|--------------------|
| homenet.basic.lan_wan | LAN и WAN | invariant | локальная сеть дома vs внешняя (интернет) | структура подключения |
...
| homenet.ip.nat | NAT | invariant | много устройств через один публичный IP | основа домашней сети |
