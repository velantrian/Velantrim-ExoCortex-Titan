...
The aggregate gate was observed end to end:

```text
Draft → pending: pull request is Draft
Ready + exact-head CI/Continuity/Docker PASS → success
```

Issue #234 remains open because repository rules do not yet require that status.
...
→ recompute signal/resolution/projection/result identities
...
Possible mappings must be accepted narrowly. Open, overdue or resolved state must not automatically become a reminder, action, current-state request or delivery authority.
...
- exact base/head SHA;
- bounded two-surface implementation + adversarial tests;
