...
- live/hot backup, production-scale restore, and disaster-recovery orchestration
  (PH-2A, MERGED #455 at `c7a34cce…`: local cold tar of the hardened production data
  ...
  is the same tar contract plus `docker-drill` in `docker.yml`; named-volume
  recovery **OBSERVED** (`ORIGINAL_STATE_ISOLATED=PASS`). **PRODUCTION NOT AUTHORIZED;**
